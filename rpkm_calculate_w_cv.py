...
def getOptions():
    ...
    args = parser.parse_args(['-m','/home/jfear/tmp/coverage/aln_to_genome.3L.mpileup','-s','/home/jfear/tmp/coverage/aln_to_genome.sam', '-b', '/home/jfear/tmp/coverage/fb551_4col.bed','-o','/home/jfear/tmp/coverage/test_genome.csv','-g','/home/jfear/tmp/coverage/test.log'])
    return(args)
...
def read_mpileup(args,bdict):
    ...
    with open(args.mname, 'r') as MPILEUP:
        ...
        for row in reader:
            ...
            mdepth = int(row[3])
            try:
                # Check if we are still in the same exonic region, if not change curr
                if curr['chrom'] == mchrom and curr['start'] <= mpos and curr['end'] > mpos:
                    loc = mpos - curr['start']
                    curr['count'][loc] = mdepth
                else:
                    for fusion in bdict:
                        if bdict[fusion]['chrom'] == mchrom and bdict[fusion]['start'] <= mpos and bdict[fusion]['end'] > mpos:
                            curr = bdict[fusion]
                            loc = mpos - curr['start']
                            curr['count'][loc] = mdepth
                            break
            except:
                # The first time around we won't have curr set, so set it
                for fusion in bdict:
                    if bdict[fusion]['chrom'] == mchrom and bdict[fusion]['start'] <= mpos and bdict[fusion]['end'] > mpos:
                        curr = bdict[fusion]
                        loc = mpos - curr['start']
...
def main():
    ...
    bdict = read_bed(args)                               # Read through BED file and create dictionary to sort all information.
    read_mpileup(args,bdict)                             # Read Mpileup file and populate the bdict with pileup information
